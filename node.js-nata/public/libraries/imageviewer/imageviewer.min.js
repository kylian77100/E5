...
!(function (i, e, t, n) {
  ...
  function o(i) {
    return (
      i.complete &&
      ("undefined" == typeof i.naturalWidth || 0 !== i.naturalWidth)
    );
  }
  function s(i, e) {
    (this.container = i),
  ...
  i(function () {
    m.length || (m = i("body")),
      m.append(
        '<div id="iv-container">' + v + '<div class="iv-close"></div><div>'
      );
  }),
      ...
      _init: function () {
        ...
        v.on("mousewheel" + r + " DOMMouseScroll" + r, function (n) {
          if (t.zoomOnMouseWheel && e.loaded) {
            ...
            var a = Math.max(
                ...
                Math.min(
                  1,
                  n.originalEvent.wheelDelta || -n.originalEvent.detail
                )
              ),
              s = (e.zoomValue * (100 + a * d)) / 100;
            if (
              (s >= 100 && s <= t.maxZoom ? (g = 0) : (g += Math.abs(a)),
              !(g > u))
            ) {
              n.preventDefault();
              var r = o.offset(),
          ...
          v.on("touchstart" + r, function (i) {
            if (e.loaded) {
              ...
              if (t && a) {
                ...
                var s = o.offset(),
                  r = Math.sqrt(
                    Math.pow(a.pageX - t.pageX, 2) +
                      Math.pow(a.pageY - t.pageY, 2)
                  ),
                  h = e.zoomValue,
                  m = {
                  ...
                  c = function (i) {
                    ...
                    var t = i.originalEvent.touches[0],
                      ...
                      a = Math.sqrt(
                        Math.pow(n.pageX - t.pageX, 2) +
                          Math.pow(n.pageY - t.pageY, 2)
                      ),
                      o = h + (a - r) / 2;
                    e.zoom(o, m);
                  },
        ...
        v.on("click" + r, function (i) {
          0 == M
            ...
            : Date.now() - M < 500 &&
              Math.abs(i.pageX - w.x) < 50 &&
              Math.abs(i.pageY - w.y) < 50
            ? (e.zoomValue == t.zoomValue ? e.zoom(200) : e.resetZoom(),
              (M = 0))
            : (M = 0);
        });
        var z,
        ...
        (D["mouseenter" + r + " touchstart" + r] = function () {
          ...
          e._fullPage &&
            (o.on(
              "touchmove" + r + " mousewheel" + r + " DOMMouseScroll" + r,
              function (i) {
                i.preventDefault();
              }
            ),
      ...
      zoom: function (i, e) {
        function t() {
          ...
          (M = Math.min(M, u)),
            ...
            n.zoomHandle.css(
              "left",
              ((h - 100) * n._zoomSliderLength) / (o - 100) + "px"
            );
        }
      ...
      load: function (e, t) {
        ...
        r.prepend('<img class="iv-snap-image" src="' + e + '" />'),
          this.imageWrap.prepend(
            '<img class="iv-large-image" src="' + e + '" />'
          ),
          t &&
            this.imageWrap.append(
              '<img class="iv-large-image" src="' + t + '" />'
            );
